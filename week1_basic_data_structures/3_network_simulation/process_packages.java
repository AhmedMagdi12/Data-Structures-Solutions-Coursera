...
class Buffer {
    ...
    public Response Process(Request request) {
        ...
        boolean dropped = (this.finish_time_.size() == this.size_) && (request.arrival_time < this.finish_time_.get(0) );
        int startTime = -1;
        if(!dropped) {
            if(finish_time_.isEmpty())
                startTime = request.arrival_time;
            else {
                int max = finish_time_.get(finish_time_.size() - 1);
                if(request.arrival_time > max)
                    max = request.arrival_time;
                startTime = max;
            }
            finish_time_.add(startTime + request.process_time);
            }
        return new Response(dropped, startTime);
    }
